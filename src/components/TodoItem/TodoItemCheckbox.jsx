...
export const TodoItemCheckbox = ({id, checked, priority}) => {
  const {mutate} = useUpdateTodoItem();  
  const onClickCheck = () => {
    /*if (checked) {
      return;
    }*/
    mutate({updatedId: id, checked: !checked, priority});
  }
  return <CheckboxContainer checked={checked} onClick={onClickCheck}/>
}